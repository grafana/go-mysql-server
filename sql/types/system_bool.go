...
func (t SystemBoolType) SQL(ctx *sql.Context, dest *bytes.Buffer, v interface{}) (sql.BufSQLValue, error) {
	if v == nil {
	...
	start := dest.Len()
	dest.Write(strconv.AppendInt(dest.AvailableBuffer(), int64(v.(int8)), 10))
